...
from flask import Flask, render_template, request, redirect, session, url_for, flash, send_file, json
...
recaptcha_site = "6LddWAgpAAAAALpyGet_KT4BAew_5NKBUVpxFg90"
recaptcha_secret = "6LddWAgpAAAAAC0tYwfoOAbW7HQdWVvE1QimJP1f"
...
def delete_bug(id_data):
    ...
    with connection.cursor() as cursor:
        ...
        cursor.execute("UPDATE bug SET is_deleted=1, deleted_at=%s WHERE bug_id=%s", (formattedDate, id_data,))
...
def recover_bug():
    ...
    return render_template('recover_bug.html',username=username,userlevel=userlevel, bugs=data)

@app.route('/recover_bug_confirm/<string:id_data>', methods = ['GET'])
def recover_bug_confirm(id_data):
    ...
    with connection.cursor() as cursor:
        #cursor.execute("DELETE FROM bug WHERE bug_id=%s", (id_data,))
        value = None
        cursor.execute("UPDATE bug SET is_deleted=0, deleted_at=%s WHERE bug_id=%s", (value, id_data,))
        
        connection.commit()
        message=f"Bug with id {id_data} was succesfully recovered"
    
        flash(message=message)
        return redirect(url_for('recover_bug'))
